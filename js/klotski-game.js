...
function updateKlotskiUI() {
    ...
    document.getElementById('klotski-best-moves').textContent = 
        (bestMoves === Infinity || !bestMoves) ? '--' : bestMoves;
    document.getElementById('klotski-best-time').textContent = 
        (bestTime === Infinity || !bestTime) ? '--' : formatTime(bestTime);
    document.getElementById('klotski-best-efficiency').textContent = 
        (bestEfficiency === Infinity || !bestEfficiency) ? '--' : (bestEfficiency.toFixed(2) + ' 秒/步');
}
        ...
        try {
            ...
            for (let level of [3, 4, 5]) {
                if (!loaded[level]) {
                    ...
                    if (!loaded[level].hasOwnProperty('moves')) loaded[level].moves = Infinity;
                    if (!loaded[level].hasOwnProperty('time')) loaded[level].time = Infinity;
                    if (!loaded[level].hasOwnProperty('efficiency')) loaded[level].efficiency = Infinity;
                }
            }
